...
void test_propagation_no_friction(void)
{
	const struct twsfwphysx_world world = { .friction = 0.F,
											.restitution = 1.F,
											.agent_radius = .1F,
	...
	for (int i = 0; i < 3; i++) {
		...
		const float v_exp = v + (a * t);
		assert(fabsf(agents.agents[0].v - v_exp) < 1e-5F);

		const float s_exp = (v * t) + (.5F * a * t * t);
...
{
	const struct twsfwphysx_world world = { .friction = 3.F,
											.restitution = 1.F,
											.agent_radius = .1F,
											.missile_acceleration = 1.F };
	...
	for (int i = 0; i < 3; i++) {
		...
		const float expmmut = expf(-world.friction * t);
		const float vt = a / world.friction;

		const float v_exp = (v * expmmut) + (vt * (1.F - expmmut));
		assert(fabsf(agents.agents[0].v - v_exp) < 1e-5F);

		const float s_exp = (vt * t) + (v / world.friction * (1.F - expmmut)) +
							(vt / world.friction * (expmmut - 1.F));
		assert(fabsf(length(agents.agents[0].r) - 1.F) < 1e-5F);
		assert(fabsf(agents.agents[0].r.x - cosf(s_exp)) < 1e-5F);
		assert(fabsf(agents.agents[0].r.z) < 1e-5F);
	}
